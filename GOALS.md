...
- [ ] **선택 노드 → LLM 종합 지식 문서** (다음 후보) — `summarize_search` 재사용, 버튼+토큰 인증, 자동 아님. 사용자 핵심 요구("쓸 수 있는 지식").
- [ ] **점수 기반 관련 필터** (다음 후보) — degree-centrality 임계로 서브그래프만 표시(검색가시성·스케일·필터를 한 번에 해결). PageRank는 후순위.
- [ ] 텔레그램 UX 개선 (별도 트랙) — 처리 진행 메시지 편집 + 완료 시 삭제 + 원본에 reaction(setMessageReaction, Bot API 7.0+ 버전 확인 필요; 안 되면 원본 편집 폴백)
