...
function docBiblioHtml(dc){
  ...
  const arxiv = (biblio.arxiv_id || '').trim();

  const items = [];
  if(author) items.push('<span class="biblio-item biblio-author">✍️ '+esc(author)+'</span>');
  if(pubAt) items.push('<span class="biblio-item biblio-date">📅 '+esc(pubAt)+'</span>');
  if(venue) items.push('<span class="biblio-item biblio-venue">🏛️ '+esc(venue)+'</span>');
  if(doi){
    const doiUrl = doi.startsWith('http') ? doi : 'https://doi.org/' + doi;
    items.push('<span class="biblio-item biblio-doi"><a href="'+esc(doiUrl)+'" target=_blank rel=noopener>DOI: '+esc(doi)+'</a></span>');
  } else if(arxiv){
    const arxivUrl = arxiv.startsWith('http') ? arxiv : 'https://arxiv.org/abs/' + arxiv;
    items.push('<span class="biblio-item biblio-arxiv"><a href="'+esc(arxivUrl)+'" target=_blank rel=noopener>arXiv: '+esc(arxiv)+'</a></span>');
  }
  if(!items.length) return '';
  return '<div class="docbiblio-row">'+items.join('<span class="biblio-sep">·</span>')+'</div>';
}
