    ...
    def render_detail(self, doc: Document) -> str:
        ...
        영향 없음. UI 가 마크다운을 렌더링하므로:
          ...
          · 원문에서 수집한 이해에 도움 되는 이미지(다이어그램·차트·스크린샷)는 적절한
            위치에 마크다운 이미지로 삽입(장식/로고/아이콘은 제외 — 큐레이션).
...
def _images_block(images: list[dict]) -> str:
    ...
    return (
        "\n[원문에서 수집한 이미지 후보]\n"
        "아래 이미지 중 글의 이해에 실제로 도움 되는 것(다이어그램·구조도·차트·스크린샷·"
        "도식)만 골라, 관련 내용 바로 옆에 마크다운 이미지 `![설명](url)` 으로 삽입하라. "
        "url 은 **목록의 값을 한 글자도 바꾸지 말고 그대로** 쓰고, 설명은 한국어로 달아라. "
        "로고·아이콘·아바타·광고·장식·중복·내용과 무관한 이미지는 넣지 마라. 쓸 만한 게 "
        "없으면 하나도 넣지 않아도 된다.\n"
        f"{listing}\n"
    )
