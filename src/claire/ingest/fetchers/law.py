...
law.go.kr 은 본문 조문을 정적 HTML 본문에 바로 담지 않고 1) 외곽 페이지(`/법령/<명칭>`, `/행정규칙/<명칭>` 등)가 iframe 으로 `*InfoP.do` 를 로드하고, 2) `*InfoP.do` 가 브라우저 JS(AJAX)로 `*InfoR.do` 를 호출해 본문을 주입하는 2중 구조를 갖는다.
