...
장시간 실행되는 배치 작업(regenerate, reextract, backfill, queue drain 등)에서 전체 진행률, 현재 대상 문서, 세부 실행 단계(LLM 추출, 본문 렌더링, 엔티티 해소/판정, 그래프 적재 등)를 실시간으로 터미널에 표시하고, 예기치 않은 중단(SIGINT/오류) 시 정확한 중단 위치와 잔여 현황, 재개 가이드를 제공합니다.
"""
