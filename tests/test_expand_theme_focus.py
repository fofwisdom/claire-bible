...
- Standalone 1-hop expansion document (same_subject=False) in a theme with default_focus:
  child document is saved with directive equal to theme's default_focus in documents.meta
...
- Theme 0 (default theme) 1-hop expansion:
  directive remains None.
- Explicit directive override:
  if caller explicitly provides directive, it overrides default_focus.
...
def test_standalone_expansion_inherits_theme_default_focus(theme_env):
    ...
    directive = dbm.get_document_directive(conn, child_id)
    assert directive == "인공지능 에이전트 및 분산 시스템 아키텍처 관점"

    row = dbm.get_document_row(conn, child_id)
    meta = json.loads(row["meta"] or "{}")
    assert meta.get("directive") == "인공지능 에이전트 및 분산 시스템 아키텍처 관점"

    detail = row["detail"]
    assert detail is not None
    assert "[directive: 인공지능 에이전트 및 분산 시스템 아키텍처 관점]" in detail
    conn.close()
...
def test_merged_expansion_applies_theme_default_focus(theme_env):
    ...
    conn = dbm.connect(theme_settings.db_file)
    directive = dbm.get_document_directive(conn, parent_rep.document_id)
    assert directive == "취약점 및 보안 위협 분석 관점"

    row = dbm.get_document_row(conn, parent_rep.document_id)
    meta = json.loads(row["meta"] or "{}")
    assert meta.get("directive") == "취약점 및 보안 위협 분석 관점"

    detail = row["detail"]
    assert detail is not None
    assert "[directive: 취약점 및 보안 위협 분석 관점]" in detail
    conn.close()


def test_theme_zero_expansion_directive_remains_none(theme_env):
    """Theme 0 (default theme) 1-hop expansion: directive remains None."""
    tm, base_settings = theme_env
    ...
    child_id = dbm.find_document_by_canonical_url(conn, CHILD_STANDALONE)
    assert child_id is not None
    assert dbm.get_document_directive(conn, child_id) is None
    row = dbm.get_document_row(conn, child_id)
    meta = json.loads(row["meta"] or "{}")
    assert meta.get("directive") is None
    assert "[directive:" not in (row["detail"] or "")
    conn.close()
    ...
    conn = dbm.connect(theme0_settings.db_file)
    assert dbm.get_document_directive(conn, parent_merge_rep.document_id) is None
    row_merge = dbm.get_document_row(conn, parent_merge_rep.document_id)
    meta_merge = json.loads(row_merge["meta"] or "{}")
    assert meta_merge.get("directive") is None
    assert "[directive:" not in (row_merge["detail"] or "")
    conn.close()


def test_explicit_directive_overrides_theme_default_focus(theme_env):
    """Explicit directive override: if caller explicitly provides directive, it overrides default_focus."""
    tm, base_settings = theme_env
    ...
    conn = dbm.connect(theme_settings.db_file)
    directive = dbm.get_document_directive(conn, rep.document_id)
    assert directive == explicit
    assert directive != theme.default_focus

    row = dbm.get_document_row(conn, rep.document_id)
    meta = json.loads(row["meta"] or "{}")
    assert meta.get("directive") == explicit
    assert f"[directive: {explicit}]" in (row["detail"] or "")
    conn.close()
