...
def test_bibliographic_row_placed_above_summary_and_separated_from_docmeta():
    """서지 정보(저자, 발행일 등)가 docmeta 가공 태그와 분리되어 요약 위에 독립된 행으로 렌더링되는지 검증."""
    ...
    # 2. docmeta 에는 가공/상태 태그만 남고 서지 정보(✍️)가 중복 혼입되지 않음
    # docMetaHtml 내부에 서지 태그(title="서지 메타데이터)가 없어야 함
    ...
    reader_render_idx = GRAPH_HTML.index("h+=docBiblioHtml(dc);")
    reader_summary_idx = GRAPH_HTML.index("<div class=rsection>요약</div>")
    assert reader_render_idx < reader_summary_idx
    ...
    # 5. CSS 스타일에 .docbiblio-row 존재
    assert ".docbiblio-row" in GRAPH_HTML
    assert ".docbiblio-row" in _SHARED_HTML
