...
def test_check_production_provider_skips_development(tmp_path):
    _write_layout(tmp_path, dev=True)
    runtime = cb.load_runtime(cb.Layout(tmp_path), legacy_dev=True)
    assert cb.check_production_provider(runtime) is None


def test_check_production_provider_warns_mock_in_production(tmp_path):
    _write_layout(tmp_path, dev=False)
    runtime = cb.load_runtime(cb.Layout(tmp_path), legacy_dev=False)
    warning = cb.check_production_provider(runtime)
    assert warning is not None
    assert "CLAIRE_PROVIDER is set to 'mock'" in warning
    assert "CLAIRE_PROVIDER=gemini" in warning
    assert "CLAIRE_PROVIDER=antigravity" in warning


def test_check_production_provider_warns_gemini_without_api_key(tmp_path):
    _write_layout(tmp_path, dev=False)
    ...
    warning = cb.check_production_provider(runtime)
    assert warning is not None
    assert "CLAIRE_PROVIDER is set to 'gemini'" in warning
    assert "GEMINI_API_KEY is not configured" in warning


def test_check_production_provider_passes_gemini_with_key(tmp_path):
    _write_layout(tmp_path, dev=False)
    env_file = tmp_path / ".env"
    env_file.write_text(
...
def test_check_production_provider_warns_antigravity_without_binary(tmp_path, monkeypatch):
    _write_layout(tmp_path, dev=False)
    env_file = tmp_path / ".env"
    env_file.write_text(
        env_file.read_text(encoding="utf-8") + "\nCLAIRE_PROVIDER=antigravity\nCLAIRE_AGY_BIN=nonexistent_agy_bin\n",
        encoding="utf-8",
    )
    ...
    warning = cb.check_production_provider(runtime)
    assert warning is not None
    assert "CLAIRE_PROVIDER is set to 'antigravity'" in warning
    assert "nonexistent_agy_bin" in warning


def test_check_production_provider_passes_antigravity_with_binary(tmp_path, monkeypatch):
    _write_layout(tmp_path, dev=False)
...
def test_check_production_provider_warns_unknown_provider(tmp_path):
    _write_layout(tmp_path, dev=False)
    env_file = tmp_path / ".env"
    env_file.write_text(
        env_file.read_text(encoding="utf-8") + "\nCLAIRE_PROVIDER=unknown_provider\n",
        encoding="utf-8",
    )
    runtime = cb.load_runtime(cb.Layout(tmp_path), legacy_dev=False)
    warning = cb.check_production_provider(runtime)
    assert warning is not None
    assert "CLAIRE_PROVIDER is set to 'unknown_provider'" in warning


def test_up_warns_when_production_provider_is_mock(tmp_path, capsys):
    _write_layout(tmp_path, dev=False)
    env_file = tmp_path / ".env"
    env_file.write_text(
    ...
    assert "cb-manuscript: warning:" in err
    assert "CLAIRE_PROVIDER is set to 'mock'" in err


def test_up_does_not_warn_when_production_provider_is_valid_gemini(tmp_path, capsys):
    _write_layout(tmp_path, dev=False)
    env_file = tmp_path / ".env"
    env_file.write_text(
    ...
    err = capsys.readouterr().err
    assert "CLAIRE_PROVIDER" not in err


def test_dev_up_does_not_warn_even_if_provider_is_mock(tmp_path, capsys):
    _write_layout(tmp_path, dev=True)
    with patch.object(cb.subprocess, "run", side_effect=_fake_success):
        assert cb.main(["dev", "up"], root=tmp_path) == 0

    err = capsys.readouterr().err
    assert "CLAIRE_PROVIDER" not in err


def test_install_warns_when_production_provider_is_mock(tmp_path, capsys):
    _write_layout(tmp_path, dev=False)
    env_file = tmp_path / ".env"
    env_file.write_text(
        env_file.read_text(encoding="utf-8") + "\nCLAIRE_PROVIDER=mock\n",
...
def test_update_warns_when_production_provider_is_mock(tmp_path, capsys):
    _write_layout(tmp_path, dev=False)
    env_file = tmp_path / ".env"
    env_file.write_text(
        env_file.read_text(encoding="utf-8") + "\nCLAIRE_PROVIDER=mock\n",
