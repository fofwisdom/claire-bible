...
advisor 조언: "better" 가 vibe 가 되지 않도록, *합쳐져야 할 쌍*과 *절대 합치면 안 될 쌍*을 명시적으로 라벨링한다. 회귀(공격적 머지로 서로 다른 도구가 뭉개짐)를 잡는다.

여기서는 임베딩을 직접 주입해 결정론적으로 검증한다(네트워크/토큰 불필요). 실제 Gemini 임베딩/판정 품질은 별도 옵트인 스크립트로 측정.
"""
