...
현재 공통 스키마는 v13이며 `schema_lineage=claire-bible/common`을 함께 검증한다.
폐기된 v12는 진단 데이터를 보존해 v11로 철회한 뒤 v13으로 승격하며, 알 수 없는 v12 서명이나 외부 계보는 변경하지 않고 실패한다.
`cb-manuscript backup`은 DB 버전과 계보를 manifest에 기록하며, `restore`는 미래 버전·외부 계보·계보 없는 현재 버전 백업을 서비스 중지 전에 거부한다.
