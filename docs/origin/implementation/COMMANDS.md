...
* `claire support-bundle [--days N] [--target <target>] [--list] [--purge] [--json]`:
  ...
  * **수집 시도 및 DOM 진단**: `tracked_document/`에 전체 인박스 행, append-only 시도 이력, static/law/discourse/scrapling/CDP 단계별 상태와 정제 HTML/최종 DOM 스냅샷을 포함한다. HTML 원본 SHA-256·길이는 보존하되 스크립트·폼 값·토큰은 제거한다.[^telemetry-implementation]
