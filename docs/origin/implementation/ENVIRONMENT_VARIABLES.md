...
| `CLAIRE_PDF_PARSER` | `pypdfium2` | `pypdfium2`, `pypdf`, `docling`, `auto` | PDF 텍스트 추출 엔진 (`pypdfium2`: 고속 Chromium C++ PDFium 파서, `pypdf`: 초경량 안전망 파서, `docling`: 딥러닝 기반 다단 칼럼/표 분석 및 OCR 파서, `auto`: pypdfium2 우선 후 결함 시 docling 에스컬레이션). ([PDF_INGESTION_AND_ADAPTIVE_EFFORT_DESIGN.md](../design/PDF_INGESTION_AND_ADAPTIVE_EFFORT_DESIGN.md) 참조) |
