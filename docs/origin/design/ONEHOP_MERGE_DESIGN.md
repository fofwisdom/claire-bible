...
## 5a. 채택하지 않은 대안 — 그래프 관계성 기반 "병합 포기 조건"

**아이디어**: 부모·자식 각각의 엔티티가 추출된 뒤 두 집합이 그래프에서 전혀 연결되지
않으면 `same_subject` 판정을 불신하고 병합을 포기한다.

익명화된 평가 표본으로 검토했을 때, 짧은 큐레이션 요약과 다른 언어로 작성된 원문처럼
실제로 같은 주제인 자료도 추출된 엔티티의 언어와 세분성이 달라 그래프 겹침이 0이 될 수
있었다. 반대로 제품 소개와 공식 저장소처럼 이미 판정하기 쉬운 조합에서는 그래프 겹침이
높아 추가 신호의 가치가 작았다.

**결론**: 그래프 관계성은 쉬운 사례를 재확인하지만 언어·표현이 다른 참양성을 거부할 수
있으므로 필수 병합 포기 조건으로 사용하지 않는다. 별도 관측 지표로는 활용할 수 있다.
