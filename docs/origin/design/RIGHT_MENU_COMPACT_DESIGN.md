...
작성일: 2026-08-21 · 상태: **구현 및 적용 (Implemented)** · 기준: [GOALS.md](../../upstream/GOALS.md) / [PLAN.md](../../upstream/PLAN.md) · 관련: `src/claire/graphview.py`
...
2. **우측 메뉴 컴팩트 레일 UI (`#detailpane.compact-rail`)**:
   ...
   - **도구 메뉴 (`#moremenu`)**:
     - 상단 일반 도구(`➕ 적재`, `♻️ 중복정리`, 검색 모드)와 **'그래프' 전용 섹션(`div#graph-section`)**으로 논리적/시각적 분리.
     - '그래프' 섹션에는 전용 기능 단추(`🔗 경로`, `🧩 종합`, 선택 칩) 및 연결 차수 슬라이더/상태 정보를 배치하고, 섹션 상단 및 하단에 명확한 구분선(`border-top`, `border-bottom`)을 적용.
     - `.action-btn-row`: 컴팩트 레일 전환 시 버튼들을 수직(Column) 중앙 정렬로 배치.
     - 버튼 텍스트(`<span class="btn-label">`) 및 섹션 타이틀(`<span class="menu-section-title">`) 숨김 처리 (`display: none`), 아이콘(`🧩`, `➕`, `♻️`, `🔗`, `🐙`)만 중앙 정렬하여 42x42px 타깃 제공.
